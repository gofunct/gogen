...
func (s *loggerSettings) apply(ctx *cobra.Command) error {
...
func AddFlags(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().StringVar(&s.level, "log-level", origLogger.Level.String(), "Only log messages with the given severity or above. Valid levels: [debug, info, warn, error, fatal]")
	cmd.PersistentFlags().StringVar(&s.format, "log-format", defaultFormat.String(), `Set the log target and format. Example: "logger:syslog?appname=bob&local=7" or "logger:stdout?json=true`)
}
